...
def extract_content_from_url(url):
    """Extract meaningful text from a webpage, prioritizing <article> and <main> sections or divs with role='main', while removing footers, navs, scripts, and irrelevant divs."""
    try:
        ...
        for tag in ["nav", "aside", "script", "style", "footer"]:
        ...
        output = url + ' ' + extracted_text[:4500]
        return output
...
def query_gpt4o(prompt):
    """Query GPT-4o via OpenAI API and return response text."""
    try:
...
def filter_relevant_content_with_llm(content, query):
    """Use GPT-4o to filter content based on relevance to the user's query."""
    prompt = f"""
...
def extract_title_description_authors(summary):
    ...
    Make sure the response is in JSON format and contains all required fields. If any information is missing, use an empty string or empty list where appropriate. Do not add any information or comment. Return only the answer in valid JSON format, do not add a ```json tag at the beginning nor at the end of your answer.
    """
    return query_gpt4o(prompt)
...
def main():
    ...
    print("\nSearching...\n")
    urls = search_google(user_query, num_results=3, exclude_sites=["researchgate.net"])
    ...
    for info in extracted_info:
        try:
            ...
            if (title == "" and description == "") : continue
            json_output = format_json_output(title, description, authors, url, year)
            print(f"json: {json_output}") # Debugging
